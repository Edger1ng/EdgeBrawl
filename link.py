...
async def start_handler(message: types.Message):
    ...
    try:
        ...
        maindb[user_id] = {'Token': "0", 'ID': 0, 'LinkLvl': 0, 'TimeEveryday': 0, "GemsWallet": 0, "TapWallet": 0, 'Transactions': {"xxx": {"Gems": 0, "TransID": "#TRxxx", "Comment": "Some, ok"}, "zzz": {"Gems": 0, "TransID": "#TRxxx", "Comment": "Some, ok"}}}
        save_data()
    ...
    if maindb[user_id]['LinkLvl'] == 0:
        ...
        if maindb[user_id]['LinkLvl'] == 2:
            ...
            builder.row(types.InlineKeyboardButton(text="Edge Wallet", callback_data="wallet"))
...
async def wallet(callback: CallbackQuery):
    ...
    id = maindb[user_id]['ID']
    player_data = db.load_player_account_by_id(id)
    gems = maindb[user_id]['GemsWallet']
    builder = InlineKeyboardBuilder()
    if maindb[user_id]['GemsWallet'] >= 1:
        builder.row(types.InlineKeyboardButton(text="Вывести Гемы", callback_data="out_gems"))
        await callback.message.answer(f"Edge Wallet - встроенный кошелек в Edge Connect.Принимайте гемы а также другие ресурсы прямо в боте\n\nГемов в кошельке: {gems}\n$EDGCOIN: 0\nID: {user_id}", reply_markup=builder.as_markup())
    else:
        await callback.message.answer(f"Edge Wallet - встроенный кошелек в Edge Connect.Принимайте гемы а также другие ресурсы прямо в боте\n\nГемов в кошельке: {gems}\n$EDGCOIN: 0\nID: {user_id}")


@dp.callback_query(F.data.lower() == 'out_gems')
async def out_gems(callback: CallbackQuery):
    user_id = str(callback.from_user.id)
    gems2 = maindb[user_id]['GemsWallet']
    id2 = maindb[user_id]['ID']
    player_data = db.load_player_account_by_id(id2)
    gems = gems2 + player_data['Gems']
    token = player_data['Token']
    db.update_player_account(token, 'Gems', gems)
    await callback.message.answer(f"Вы успешно вывели {gems2} гемов.\n\nКоммисия: 0%")

@dp.message(Command("pay"))
async def pay_main(message: types.Message):
    user_id = str(message.from_user.id)
    try:
        arg1 = str(message.text.split(' ')[1])
        arg2 = str(message.text.split(' ')[2])
        arg3 = str(message.text.split(' ')[3])
    except Exception:
        await message.answer(f"/pay - команда для отправки гемов и прочей валюты через систему Edge Connect\n\nПримеры:\n/pay 18439 100 gems - отправка 100 гемов на Edge Wallet игрока с ID 18439\n\nВаш ID: {user_id}")
        return
    ...
    if arg3 == "gems":
        if int(arg2) > int(player_data['Gems']):
            await message.reply("Операция отклонена со стороны системы.\n\nПричина: недостаточно гемов на аккаунте.")
        else:
            await message.reply("Проводим операцию...")
            try:
                acc = maindb[arg1]['GemsWallet']
                stat = 1
            except Exception:
                await message.reply("Ошибка!\nДанного человека нет в Edge Connect!")
                return
            
            if stat == 1:
                gems1 = int(player_data['Gems'])
                gems2 = int(arg2)
                gems1 -= gems2
                token = player_data['Token']
                
                db.update_player_account(token, 'Gems', gems1)
                maindb[arg1]['GemsWallet'] = acc + gems2  # обновление кошелька получателя
                save_data()
                
                await message.reply("Операция успешно завершена!")
            else:
                await message.reply("Операция была отклонена системой.")
    else:
        await message.reply("Похоже, данная валюта еще не поддерживается Edge Pay")
...
async def privacy(message: types.Message):
    await message.reply("Это - команда о том, какие данные хранит Edge Connect и каким образом он их хранит.\n\nЭто команда была созданапо требованию Telegram.\n(Дурев дура)")
    await message.reply("Какие данные хранятся:\n\nВаш ID Telegram\nВаш TOKEN в Edge Brawl(Иногда динамически подтягивается с серверов)\nВаш ID В Edge Brawl\n\n")
    await message.reply("Как хранятся данные:\n\n-На SSH сервере под паролем из 12 цифр\n-Данные копируются на другой сервер каждые 7 дней")
    await message.reply("Edge Connect(В частности, команда проектов Edge Team) предоставляет возможность полного удаления аккаунта с серверов, однако это занимает до 30 дней.")
