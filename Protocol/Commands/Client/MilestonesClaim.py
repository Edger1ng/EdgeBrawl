...
from ByteStream.Reader import BSMessageReader
from Protocol.Commands.Client.MilestonesClaimHelp import MilestonesClaimHelp as Help
from Protocol.Commands.Client.MilestonesClaimSupply import MilestonesClaimSupply as Supply
import random as rnd
class MilestonesClaimBy(BSMessageReader):
    ...
    def process(self):
        ...
        if self.Road in [6]:
            ...
            DB.update_player_account(self.player.token,'Troproad', self.player.Troproad)
        import json
        if self.Road in [9] and self.Season in [1]:
            ...
            DB.update_player_account(self.player.token,'buypass', self.player.buypass)
        if self.Road in [10] and self.Season in [1]:
            ...
            DB.update_player_account(self.player.token ,'freepass', self.player.freepass)
